...
struct Request process_request(char req_buffer[]) {
    ...
    if ((check = sscanf(p_buf, "%s /%s %s %n", req.method, req.uri, req.version, &req.off_set))
        != 3) {
    ...
    if (!((strcmp(req.method, "GET") == 0 | strcmp(req.method, "get") == 0)
            || (strcmp(req.method, "PUT") == 0 | strcmp(req.method, "put") == 0))) {
        req.err_flag = 501;
        return req;
    }
...
void handle_connection(int connfd) {
    ...
    if (req.err_flag == 501) {
        ...
        dprintf(
            connfd, "HTTP/1.1 501 Not Implemented\r\nContent-Length: 16\r\n\r\nNot Implemented\n");
        return;
    }
